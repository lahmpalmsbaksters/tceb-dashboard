...
const AnalyticsCounter = ({ title, value, icon, subTitle }) => {
  return (
    <div className="bg-white shadow-xl p-4 sm:p-6 lg:p-8 rounded-xl min-w-[300px] min-h-[150px] flex flex-col justify-between">
      {/* First Row: Title and Subtitle */}
      <div>
        <div className="text-sm sm:text-lg font-bold md:text-3xl">{title}</div>
        <div className="text-xs sm:text-sm text-gray-500 md:text-xl">{subTitle}</div>
      </div>
